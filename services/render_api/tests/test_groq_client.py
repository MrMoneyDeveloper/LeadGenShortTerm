...
def setup_client(cfg, monkeypatch, responses):
    ...
    cfg.groq_daily_candidate_cap = 2
    ...
    monkeypatch.setattr(groq, "reserve", lambda *args: reservations.append(args) or True)
...
def test_groq_strict_payload_redaction_and_estimated_accounting(isolated_settings, monkeypatch):
    ...
    assert reservations == [("groq", 1, 2)]
    assert accounting == [("groq", 100, 20, 0.00014)]


def test_groq_rate_retry_cap_and_strict_rejection(isolated_settings, monkeypatch):
    ...
    monkeypatch.setattr(groq, "reserve", lambda *_args: False)
    with pytest.raises(groq.GroqUnavailable, match="groq_daily_cap"):
        groq.classify("Ambiguous", 80)
    assert not calls
